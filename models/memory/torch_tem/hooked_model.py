...
class Model(torch.nn.Module):
    def __init__(self, params, patch_size):
    ...
    def forward(self, walk, prev_iter = None, prev_M = None):
        ...
        for i, (g, x, a) in enumerate(walk):
            ...
            if i > 0:
                stats = self.hooked_iteration(x, g, steps[-1].a, steps[-1].M, steps[-1].x_inf, steps[-1].g_inf)
                self.activation_log += stats
    ...
    def hooked_iteration(self, x, locations, a_prev, M_prev, x_prev, g_prev):
        ...
        output_data = []
        for stamp_loc in range(0, 45, self.patch_size):
            new_x = torch.zeros_like(x)
            new_x[:, stamp_loc:stamp_loc+self.patch_size] = 1
            x_inf, g_inf, p_inf_x, p_inf = self.inference(new_x, locations, M_prev, x_prev, gt_inf)
            output_data += [(stamp_loc, [x.detach() for x in p_inf])]
    ...
    def inference(self, x, locations, M_prev, x_prev, g_gen):
        ...
        p = self.inf_p(x_, g_) # <<<< @TODO -- pretty sure this is where we apply mask? >> ex: "imagine" we are in each slot, how does this change output?
