"""
Code forked from https://github.com/erkil1452/touch/tree/master/classification
Modified TouchNet to return activations after CNN layer
"""
...
from .resnet_3x3 import resnet18
...
class TouchNet(nn.Module):
    ...
    def forward(self, x, net_only=False):
        xs = []
...
class ClassificationModel(BaseModel):
    ...
    def step(self, inputs, isTrain = True, params = {}):
        ...
        image = torch.autograd.Variable(inputs['image'].to('cuda', non_blocking=True), requires_grad = (isTrain))
        pressure = torch.autograd.Variable(inputs['pressure'].to('cuda', non_blocking=True), requires_grad = (isTrain))
        objectId = torch.autograd.Variable(inputs['objectId'].to('cuda', non_blocking=True), requires_grad=False) if 'objectId' in inputs else None
